...
public class Services {
    public void PersonalityClassifier(Model.Participant p,Scanner sc){
        ...
        Model.PersonalityType type;
        if (score >= 90){
            type = Model.PersonalityType.Leader;
        }else if (score >= 70){
            type = Model.PersonalityType.Balanced;
        }else if (score >= 50){
            type = Model.PersonalityType.Thinker;
        }else{
            type = Model.PersonalityType.Thinker;
        }
        p.setType(type);
    ...
    public Model.Participant ParticipantInfo(Scanner sc) {
